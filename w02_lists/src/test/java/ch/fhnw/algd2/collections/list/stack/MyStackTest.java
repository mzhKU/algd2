...
public class MyStackTest {
	...
	private void checkPopOrder(int[] numbers) {
		for (int i = numbers.length - 1; i >= 0; i--) {
			assertFalse("Stack should not be empty as long as not all elements have been poped", stack.empty());
			assertEquals(numbers[i], stack.pop().intValue());
		}
		assertTrue("Stack should be empty again after removing all elements", stack.empty());
	}
