...
public class D_MyLinkedListTest_Complete extends AbstractMyLinkedListTest {
	...
	public void add_ElementAtIndex2_AddedAtIndex2() {
		...
		checkOrderByReference(expectedNumbers, list);
	}
	...
	private void checkOrderByReference(Integer[] numbers) {
		checkOrderByReference(numbers, list);
	}

	private <T> void checkOrderByReference(T[] numbers, MyAbstractList<T> list) {
		assertEquals(list.size(), numbers.length);
		Object[] listArray = list.toArray();
		for (int i = 0; i < numbers.length; i++) {
			assertSame("Please add elements at the end of the list (default behaviour for lists add method)", numbers[i],
					listArray[i]);
		}
	}
}
