...
import ch.fhnw.algd2.binsearchtrees.kapitel3.BinaryNode;
...
public class BinSearchTree {
	...
	public boolean search(BinSearchTree node, int key) {
	    if (node != null) {
			...
			if (key > node.getKey()) { return search(node.getRight(), key); }
			else {
				System.out.print("|");
	...
	public BinSearchTree insert(BinSearchTree v, int key) {
		if (v == null) {
			v = new BinSearchTree(key);
		} else {
			System.out.print(v.getKey() + " ");
			if (key < v.getKey()) {
				v.setLeft(insert(v.getLeft(),  key));
			} else {
				v.setRight(insert(v.getRight(), key));
			}
		}
		return v;
	}
